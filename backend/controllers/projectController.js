...
const createProject = async (req, res) => {
  try {
    ...
    // Admin ko members list me add karo agar already nahi hai (admin khud confirm hai, invite nahi lagti)
    if (!existingMembers.some(m => m.id === projectadmin)) {
      const admin = await userModel.findById(projectadmin);
      if (admin) {
        existingMembers.push({ id: admin.id, name: admin.name });
    ...
    // FIX/NEW BEHAVIOR: project banate waqt sirf ADMIN turant member banta hai.
    // Baaki sab logo ko turant add nahi kiya jaata — unhe ek "invite" (pending request) bheji jaati hai.
    // Wo sirf tab project ka hissa banenge jab accept karenge, ya 24 ghante me koi response na de.
    const adminInfo = existingMembers.find(m => m.id === projectadmin);
    ...
    const project = await projectModel.createProject({
      ...
      projectadmin,
      created_at,
      members: [adminInfo], // sirf admin turant member banta hai
    });

    // Baaki members ko invite bhejo
    const otherMembers = existingMembers.filter(m => m.id !== projectadmin);
    ...
    for (const member of otherMembers) {
      const invite = await inviteModel.createInvite({
        ...
        invited_by: projectadmin,
        created_at,
        expires_at,
      });
      sentInvites.push({ ...invite, name: member.name });
    }
    ...
    return res.status(201).json({
      ...
      Data: {
        ...
        message: `Project created. ${sentInvites.length} invite(s) sent, waiting for acceptance.`,
      },
    });
  } catch (err) {
    console.error('Create project error:', err);
...
// NOTE: addProjectMember (direct add) hata diya gaya hai — ab member add karne
// ke liye inviteController.sendInvite use hota hai (routes/projects.js me wired hai)
