...
const init = () => {
  ...
  // NEW: invite/request system — member sirf tab add hota hai jab wo accept kare,
  // ya 24 ghante ke andar koi response na de (to auto-accept ho jata hai)
  const createProjectInvitesTable = `
  ...
  db.serialize(() => {
    ...
    db.run(createProjectInvitesTable, err => {
      if (err) console.error('Error creating project_invites table:', err);
    });
    db.run(createExpensesTable, err => {
      if (err) console.error('Error creating expenses table:', err);
    });
